...
dataset_root = "/mnt/disk2/kitti-dataset/raw_data"
seq_names = ['2011_09_26_drive_0091_sync']
dates = [seq_name[:10] for seq_name in seq_names]
camera_folder = "image_02"
image_folder = "data"
result_root = "/mnt/ssd2/maskrcnn/results/kitti/raw_data"

for seq_id in tqdm(range(len(seq_names))):
    date = dates[seq_id]
    seq_name = seq_names[seq_id]
    seq_dir = os.path.join(dataset_root, date, seq_name, camera_folder, image_folder)
    result_dir = os.path.join(result_root, date, seq_name, camera_folder)
    ...
    for idx in range(len(img_files)):
        ...
        cv2.imwrite('{}/{}'.format(result_dir, img_files[idx]), pred_img)
